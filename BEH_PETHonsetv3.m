...
function [results] = BEH_PETHonsetv3(contvar, fscontvar,behvar, evsamp, fsev, combiname, savepath, varargin)
...
for nn = 1:size(BLdata)
    BLmed(nn) = median(nanBLdata(nn, :), 'omitnan'); BLmad(nn,1) = mad(nanBLdata(nn, :),1);
    EVdataZ(nn, :) = (nanEVdata(nn, :) - BLmed(nn))./BLmad(nn,1);  % contvar on baseline
% WITH NORMALIZATION OF BL:
    BLdataZ(nn, :) = (nanBLdata(nn, :) - BLmed(nn))./BLmad(nn,1);
end
...
meanBL = nan(size(binnedBL, 1), 1); 
medianBL = meanBL; madBL = meanBL; maxBL = meanBL;
meanEV = meanBL; medianEV = meanBL; madEV = meanBL; maxEV = meanBL;

for jj = 1:size(binnedBL, 1)
    if double(sum(~isnan(binnedBL(jj, :))))*PETHfsev < 0.5 || sum(isnan(binnedEV(jj, :))) == size(binnedEV(jj, :), 2) || sum(~isnan(binnedEV(jj, :))) == 1 % criterio de duración.. si ponemos una fsev para el PETH muy alto, hay que tener cuidado porque podría pasar mucho esto al ser un ds
        tbl(jj) = sum(~isnan(binnedBL(jj, :)))*PETHfsev; AUCbl(jj) = nan;
        densbl(jj) = nan; AUCev(jj) = nan; densev(jj) = nan;
        meanBL(jj) = nan; meanEV(jj) = nan; medianBL(jj) = nan; medianEV(jj)=nan;
        madBL(jj) = nan; madEV(jj) = nan; maxBL(jj) = nan; maxEV(jj) = nn;
    else
    ...
    % Other measures for BL:
    meanBL(jj) = mean(binnedBL(jj, ~isnan(binnedBL(jj, :))), 'omitnan');
    medianBL(jj) = median(binnedBL(jj, ~isnan(binnedBL(jj, :))), 'omitnan');
    madBL(jj) = mad(binnedBL(jj, ~isnan(binnedBL(jj, :))), 1);
    maxBL(jj) = max(binnedBL(jj, ~isnan(binnedBL(jj, :))),[], 'omitnan');
    % Other measures for event:
    meanEV(jj) = mean(binnedEV(jj, ~isnan(binnedEV(jj, :))), 'omitnan');
    medianEV(jj) = median(binnedEV(jj, ~isnan(binnedEV(jj, :))), 'omitnan');
    madEV(jj) = mad(binnedEV(jj, ~isnan(binnedEV(jj, :))));
    maxEV(jj) = max(binnedEV(jj, ~isnan(binnedEV(jj, :))),[], 'omitnan');
...
% evsampthresh = evsamp(~underthresh, :);
% boutdurthresh = boutdur(~underthresh, :);
behmeandata = size(evsamp, 1);
behmediandata = behmeandata;
behstddata = behmeandata;
behmaxdata = behmeandata;

for ii = 1:size(evsamp, 1)
    if double(sum(~isnan(binnedBL(ii, :))))*PETHfsev < 0.5 || sum(isnan(binnedEV(ii, :))) == size(binnedEV(ii, :), 2) % criterio de duración.. si ponemos una fsev para el PETH muy alto, hay que tener cuidado porque podría pasar mucho esto al ser un ds
        behmeandata(ii) = nan; behmediandata(ii) = nan; 
        behstddata(ii) = nan; behmaxdata(ii)= nan; 
    else
    behmeandata(ii) = mean(behvar(evsamp(ii, 1):evsamp(ii, 2)), 'omitnan'); % duración bin en seg // la dur del bout ya la tenemos
    behmediandata(ii) = median(behvar(evsamp(ii, 1):evsamp(ii, 2)), 'omitnan'); % duración bin en seg // la dur del bout ya la tenemos
    behstddata(ii) = std(behvar(evsamp(ii, 1):evsamp(ii, 2)), 'omitnan'); % duración bin en seg // la dur del bout ya la tenemos
    behmaxdata(ii) = max(behvar(evsamp(ii, 1):evsamp(ii, 2)),[],'omitnan'); % duración bin en seg // la dur del bout ya la tenemos    
    end
end
...
for ii = 1:AUCqt
    for jj = 1:size(BLdataZ, 1)
        if double(sum(PETHts >= AUCint(ii, 1) & PETHts < AUCint(ii, 2) & ~isnan(PETHev(jj, :))))*bin < 0.5
            AUCdata(jj, ii) = nan; DENSdata(jj, ii) = nan; meandata(jj, ii)  = nan; mediandata(jj, ii)  = nan;
            maxdata(jj, ii)  = nan; maddata(jj, ii)  = nan;
...
measurenames1 = {'AUCpre', 'AUCpost', 'DENSpre (Apre/n)', 'DENSpost(Apost/n)', 'durpre (s)', 'boutdur (s)',...
    'Mean Pre', 'Mean Post', 'Median Pre', 'Median Post', 'Mad Pre', 'Mad Post', 'Max Pre', 'Max Post'};
measurenames12 = {'Mean (cm/s)', 'Median (cm/s)', 'Std (au)', 'Max (cm/s)', 'boutdur'};
prepostFP = table(AUCbl.', AUCev.', densbl.', densev.', tbl.', boutdur, ...
       meanBL, meanEV, medianBL, medianEV, madBL,madEV,maxBL,maxEV, ...
       'VariableNames', measurenames1);
prepostBEH = table(behmeandata.', behmediandata.', behstddata.', behmaxdata.',boutdur, 'VariableNames', measurenames12);
...
xlsx12 = strcat(savepath, '/Data', combiname, '/onsetPETH/prepostBEH.xlsx');
...
writetable(prepostBEH, xlsx12, 'WriteVariableNames', true); 
